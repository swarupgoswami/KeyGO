...
const Sidebar = () => {
  ...
  return (
    <div className='bg-yellow-500 relative min-h-screen md:flex flex-col items-center pt-8 max-w-13 md:max-w-50 w-full border-r border-bordercolor text-sm'>

        <div className="group reltaive ">
            <label htmlFor="image">
                <img className='' src={image ? URL.createObjectURL(image):user?.image } alt="" />
        ...
        {image && ( <button className='absolute top-0 right-0 flex p-2 gap-1 bg-primary/10 text-primary cursor-pointer'>save <img src={assets.check_icon} width={13} onClick={updateImage} alt="check icon"  /></button>)}
